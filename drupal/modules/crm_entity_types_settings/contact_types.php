...
function crm_entity_types_settings_create_panels_tab($tab)
{
	$tab['page callback'] = 'crm_entity_types_settings_household_cb';
...
function crm_entity_types_settings_create_views_tab($tab, $url_components)
{
	...
	foreach($url_components as $url_component)
	{
		...
		if (is_numeric($url_component[2])){
			$tab['page arguments'][$url_component[2]] = $url_component[0] + 1; //+1 because we add $url_base in the beginning and it consists of no slashes (comment AAA)
		}
	}
